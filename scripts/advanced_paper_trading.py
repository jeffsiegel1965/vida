...
class MockMainnetEndpoint:
    ...
    async def simulate_send(self, to_address: str, amount: float) -> dict:
        ...
        if not is_valid_addr:
            return {
                "success": False,
                "error": f"Invalid address format: {to_address}",
...
class AdvancedPaperTrading:
    ...
    async def scenario_2_address_validation_comprehensive(self):
        ...
        for address in test_addresses:
            ...
            expected_valid = address.startswith("kaspa:") and len(address) == 61 and "invalid" not in address and "qqqqq" not in address and address != ""
    ...
    async def scenario_4_network_resilience(self):
        ...
        for i in range(20):
            ...
            balance_queries.append({
                "query": i + 1,
                "balance": balance,
                "response_time_ms": round(query_time * 1000, 1)
            })
