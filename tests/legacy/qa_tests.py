...
def test_t1_wallet_creation():
    ...
    assert w_test.address.startswith("kaspatest:"), (
        f"Testnet address should start with 'kaspatest:', got: {w_test.address}"
    )
...
def test_t12_daily_limit():
    ...
    assert w.sign_with_session(session.public_key_hex, "tx3", 10.0) is None, (
        "3rd 10 KAS should be rejected (over daily cap)"
    )
